...
class layer_inner_z(keras.layers.Layer):
    def __init__(self, m_2:int, activation = keras.layers.LeakyReLU(alpha=0.01), weight_initializer=tf.random_uniform_initializer(minval=0., maxval=1.0), **kwargs):
    ...
    def call(self, input):
        ...
        positive_parameters = tf.where(make_positive < 0, tf.zeros_like(make_positive), make_positive) #TODO: Can one differentiate this? Test a negative case!
        ...
        assert first_summand.shape == [z_1.shape[0], self.m_2, 1], "first summand shape is {}".format(first_summand.shape) + " expected it to be {}".format([self.m_2, 1])
        second_summand = tf.matmul(self.W_1_y, tf.multiply(y, tf.matmul(self.W_1_yu, u_1) + self.b_1_u))
        assert second_summand.shape == [z_1.shape[0], self.m_2, 1], "second summand shape is {}".format(second_summand.shape) + " expected it to be {}".format([self.m_2, 1])
        third_summand = tf.matmul(self.W_1_u, u_1) + self.b_1
        assert third_summand.shape == [z_1.shape[0], self.m_2, 1], "third summand shape is {}".format(third_summand.shape) + " expected it to be {}".format([self.m_2, 1])
        output = first_summand + second_summand + third_summand
        assert output.shape == [z_1.shape[0], self.m_2, 1], "output shape is {}".format(output.shape) + " expected it to be {}".format([self.m_2, 1])
...
if __name__ == "__main__":
    ...
    X_train_batch = (tf.random.uniform([10,3,1]), tf.random.uniform([10,2,1]), tf.random.uniform([10,3,1]))
